...
func RegisterDexpreoptCheckBuildComponents(ctx android.RegistrationContext) {
	ctx.RegisterModuleType("dexpreopt_systemserver_check", dexpreoptSystemserverCheckFactory)
}
...
func dexpreoptSystemserverCheckFactory() android.Module {
...
func (m *dexpreoptSystemserverCheck) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	if ctx.ModuleName() != "dexpreopt_systemserver_check" || ctx.Namespace().Path != "." {
		ctx.ModuleErrorf(`dexpreopt_systemserver_check can only be used by a single module named "dexpreopt_systemserver_check" in the root namespace.`)
	}
...
func (m *dexpreoptSystemserverCheck) MakeVars(ctx android.MakeVarsModuleContext) []android.ModuleMakeVarsValue {
	return []android.ModuleMakeVarsValue{{"DEXPREOPT_SYSTEMSERVER_ARTIFACTS", strings.Join(m.artifacts, " ")}}
}
