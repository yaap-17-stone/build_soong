...
type DeviceProperties struct {
	...
	// Name of the system partition filesystem module that is generated by soong.
	// This is used to check the parity between the handwritten system partition and the
	// auto-generated system partition. So it must not be included in the PartitionNameProperties.
	System_filelist_diff_ref *string
}
...
type fileListCheckDepTagType struct {
	blueprint.BaseDependencyTag
}
...
var fileListCheckDepTag fileListCheckDepTagType
...
func (a *androidDevice) DepsMutator(ctx android.BottomUpMutatorContext) {
	...
	if a.deviceProps.System_filelist_diff_ref != nil {
		ctx.AddDependency(ctx.Module(), fileListCheckDepTag, *a.deviceProps.System_filelist_diff_ref)
	}
...
func (a *androidDevice) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if diff := a.buildSystemFileListDiff(ctx); diff != nil {
		validations = append(validations, diff)
		ctx.Phony("compare_system_filelists", diff)
	}
...
func CreateFileListDiffTest(ctx android.ModuleContext, diffTestResultFile android.WritablePath, makeFileList android.Path, soongFileList android.Path, soongPartitionModuleName string) android.Path {
	builder := android.NewRuleBuilder(pctx, ctx).SandboxDisabled()
	builder.Command().BuiltTool("file_list_diff").
		Input(makeFileList).
		Input(soongFileList).
		Text(soongPartitionModuleName)
	builder.Command().Text("touch").Output(diffTestResultFile)
	builder.Build(soongPartitionModuleName+" diff test", "Installed file list diff test")
	return diffTestResultFile
}

func GetFileListFile(ctx android.ModuleContext, partitionName string, tag blueprint.DependencyTag) android.Path {
	module := ctx.GetDirectDepProxyWithTag(partitionName, tag)
	filesystemInfo, ok := android.OtherModuleProvider(ctx, module, FilesystemProvider)
	if !ok {
		ctx.ModuleErrorf("Expected dependency %s to be a filesystem", partitionName)
		return nil
	}
	return filesystemInfo.FileListFile
}

func (a *androidDevice) buildSystemFileListDiff(ctx android.ModuleContext) android.Path {
	if a.deviceProps.System_filelist_diff_ref == nil {
		return nil
	}

	var handwrittenFileList android.Path
	var systemModuleName string
	if a.partitionProps.System_partition_name != nil {
		systemModuleName = *a.partitionProps.System_partition_name
		handwrittenFileList = GetFileListFile(ctx, systemModuleName, filesystemDepTag)
	} else if info, ok := a.superImageInfo.SubImageInfo["system"]; ok {
		handwrittenFileList = info.FileListFile
		systemModuleName = info.ModuleName
	} else {
		return nil
	}

	generatedFileList := GetFileListFile(ctx, *a.deviceProps.System_filelist_diff_ref, fileListCheckDepTag)
	diffTestTimestamp := android.PathForModuleOut(ctx, "system_filelist_diff.timestamp")
	return CreateFileListDiffTest(ctx, diffTestTimestamp, generatedFileList, handwrittenFileList, systemModuleName)
}
