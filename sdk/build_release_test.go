...
func TestNameToRelease(t *testing.T) {
	...
	t.Run("invalid release", func(t *testing.T) {
		...
		android.FailIfNoMatchingErrors(t, `unknown release "A", expected one of \[S,Tiramisu,UpsideDownCake,VanillaIceCream,Baklava,CinnamonBun,F1,F2,current\]`, []error{err})
	})
}
...
func TestParseBuildReleaseSet(t *testing.T) {
	...
	t.Run("closed range", func(t *testing.T) {
		...
		android.AssertDeepEquals(t, "errors", nil, err)
		android.AssertStringEquals(t, "set", "[S,Tiramisu,UpsideDownCake,VanillaIceCream,Baklava,CinnamonBun,F1]", set.String())
	})
